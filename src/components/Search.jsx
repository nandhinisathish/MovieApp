
const Search = (props) => {

    const{searchValue, setSearchValue} = props;

  return (
    <form>
      <img src="data:image/png;base64," />
      <input type="text" value={searchValue} onChange={(e)=>setSearchValue(e.target.value)} placeholder="Search through thousands of movies" />
      <button className="text-3xl text-red-500">Search</button>
    </form>
  );
};

export default Search;
